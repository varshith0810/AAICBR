...
def render_signin(request: Request, message: str = "") -> str:
...
def render_create_account(request: Request, message: str = "") -> str:
    flash = f"<p>{message}</p>" if message else ""
...
def render_result(request: Request, top: str, conf: float, animal_id: str, location_label: str, rows: str, image_b64: str) -> str:
...
def resolve_location_label(gps_coordinates: str) -> str:
    ...
    try:
        ...
        if village_or_town:
            return ", ".join([p for p in [village_or_town, state, country] if p])
...
def inspect_bundle_files() -> dict[str, Any]:
    bundle = Path(os.getenv("MODEL_BUNDLE", "cattle_model_low_hw.tar.gz"))
    info: dict[str, Any] = {"bundle_path": str(bundle), "exists": bundle.exists(), "files": []}
